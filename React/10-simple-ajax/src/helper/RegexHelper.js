...
class RegexHelper {
    ...
    /**
     * 값의 존재 여부를 검사한다.
     * @param {HTMLElement} field 검사할 값
     * @param {String} msg      값이 없을 겨웅 표시할 메세지 내용
     */
    value(field, msg) {
     ...
     * @param {string} msg         값이 없을 경우 표시 될 메세지
     */
    ...
    /**
     * 두 값이 동일한지 검사한다.
     * @param {HTMLElement} origin      원본
     * @param {HTMLElement} compare     검사대상
     ...
     * @param {NodeList} field     검사할 CheckBod에 대한 컬랙션
     * @param {string} msg 검사에 실패할 경우 표시할 메세지
     */
    check(field, msg) {
    ...
    /** 
     * 라디오나 체크박스의 최소 선택 갯수를 제한한다.@@param {NodeList} field     검사할 CheckBod에 대한 컬랙션
     * @param {string} msg 검사에 실패할 경우 표시할 메세지
     */
   ...
   /**
    * 핸드폰번호 형식과 집전화번호 형식 둘중 하나를 충족하는지 검사
    * @param {HTMLElement} field     검사할 대상에 대한 <input>요소의 DOM 객체
